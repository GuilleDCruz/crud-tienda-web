...
export class CategoriaModal {
  ...
  guardar(): void {
    ...
    if (!this.nombre.trim()) {
      this.mensajeError.set('El nombre de la categoría es obligatorio.');
      return;
    }
    if (!this.descripcion.trim()) {
      this.mensajeError.set('La descripción de la categoría es obligatoria.');
      return;
    }
  ...
  private crear(request: CategoriaRequest): void {
    this.categoriaService.guardar(request).subscribe({
      ...
      error: (error) => {
        ...
        this.cargando.set(false);
        this.mensajeError.set(error.error?.message ??'No fue posible crear la categoría.');
      }
    });
  }
  ...
  private actualizar(request: CategoriaRequest): void {
    ...
    this.categoriaService.actualizar(this.categoria.id, request).subscribe({
        next: () => {
          this.cargando.set(false);
          this.categoriaGuardada.emit();
          this.cerrar.emit();
        ...
        error: (error) => {
          ...
          this.cargando.set(false);
          this.mensajeError.set(error.error?.message ??'No fue posible actualizar la categoría.');
        }
      });
  }
